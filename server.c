...
if(username_bit==0){
strcat(userlist,msgBuffer);
strcat(userlist,"\n");
for(int i=0;i<10;i++){
if(user_fd[i]==0){
strcpy(user[i],msgBuffer);
user_fd[i]=clientSocketFd;
break;
}
}
username_bit=1;
...
/*else if(strstr(msgBuffer,"-f")!=NULL){
...
*/
else if(strcmp(msgBuffer, "ls\n")==0){
strcpy(msgBuffer,userlist);
while(q->full)
            {
                pthread_cond_wait(q->notFull, q->mutex);
            }
...
if(strcmp(msgBuffer, "y\n")==0){
char *ptr=NULL;
ptr=strstr(store,"-f");
*ptr='f';
strcpy(msgBuffer,store);
strcpy(store,"");
}
else{
strcpy(store,"");
...
for(int i=0;i<10;i++){
	...
	if(strstr(msg,c)!=NULL){
		int socket = user_fd[i];

if(strstr(msg,"-f")!=NULL){
strcpy(store,msg);
store_bit=1;
strcpy(msg,"Do you want to recieve?\n");
}
...
/*if(strstr(msg,"-f")!=NULL){
	...
	*clean='\0';	
	...
	while(!feof(fp)){
		...
		numbytes = write(socket, buf, numbytes);
